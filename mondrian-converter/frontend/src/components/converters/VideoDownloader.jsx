// src/components/converters/VideoDownloader.jsx
...
const VideoDownloader = () => {
  ...
  const iconProps = {
    color: 'white',
    style: { position: 'absolute', opacity: 0, filter: 'drop-shadow(2px 2px 2px rgba(0,0,0,0.3))' },
  };
    ...
    <div style={{ 
      ...
      <div style={{ position: 'absolute', top: '2rem', left: '2rem', zIndex: 100 }}>
        <button onClick={() => navigate('/')} style={{
            ...
            fontFamily: 'Audiowide', letterSpacing: '2px'
          }}>
      ...
      <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '900px', height: '600px' }}>
        ...
        {/* Left shapes with floating icons */}
        <div style={{
          position: 'absolute', left: '50px', top: '120px', width: '140px', height: '220px',
          transform: 'skewY(-8deg) rotate(5deg)', zIndex: 5, overflow: 'hidden', border: '5px solid black', background: '#0047AB'
        }}>
            <Music {...iconProps} size={30} style={{ ...iconProps.style, bottom: -40, left: 20, animation: 'floatUp 8s linear 0s infinite' }} />
            <Disc {...iconProps} size={35} style={{ ...iconProps.style, bottom: -40, left: 80, animation: 'floatUp 9s linear 2s infinite' }} />
            <Music2 {...iconProps} size={30} style={{ ...iconProps.style, bottom: -40, left: 50, animation: 'floatUp 7s linear 5s infinite' }} />
        </div>
        ...
        <div onClick={() => navigate('/media-machine/convert-media')} style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)', width: '420px', height: '380px', zIndex: 40, animation: 'pulse 3s ease-in-out infinite', cursor: 'pointer' }}>
          ...
          {/* Arrow shaft - NO TOP BORDER for seamless look */}
          ...
          {/* CONVERT text in triangle */}
          <div style={{ position: 'absolute', left: '50%', top: '78px', transform: 'translateX(-50%)', textAlign: 'center', zIndex: 45, pointerEvents: 'none' }}>
            <div style={{ fontSize: '2.6rem', color: 'white', textTransform: 'uppercase', textShadow: '3px 3px 0px rgba(0,0,0,0.3)', letterSpacing: '2px', fontFamily: 'Audiowide', fontWeight: 700 }}>
              CONVERT
            </div>
          </div>
          {/* VIDEO ICON in rectangle */}
          <div style={{ position: 'absolute', display: 'flex', justifyContent: 'center', alignItems: 'center', left: '50%', top: '190px', width: '160px', height: '180px', transform: 'translateX(-50%)', zIndex: 45, pointerEvents: 'none' }}>
            <div style={{ filter: 'drop-shadow(3px 3px 0px rgba(0,0,0,0.3))' }}>
              <Play size={70} strokeWidth={2.5} fill="white" color="white" />
            </div>
          </div>
        </div>

        {/* Right shapes with floating icons */}
        <div style={{
          position: 'absolute', right: '80px', top: '100px', width: '150px', height: '240px',
          transform: 'skewY(10deg) rotate(-10deg)', zIndex: 6, overflow: 'hidden', border: '5px solid black', background: '#0047AB'
        }}>
            <Disc {...iconProps} size={40} style={{ ...iconProps.style, bottom: -40, left: 10, animation: 'floatUp 10s linear 1s infinite' }} />
            <Music {...iconProps} size={30} style={{ ...iconProps.style, bottom: -40, left: 90, animation: 'floatUp 7s linear 3s infinite' }} />
            <Music2 {...iconProps} size={35} style={{ ...iconProps.style, bottom: -40, left: 45, animation: 'floatUp 8s linear 6s infinite' }} />
        </div>
        ...
        <div style={{ position: 'absolute', right: '-20px', top: '280px', width: '300px', height: '65px', background: '#F7D002', border: '5px solid black', transform: 'rotate(-8deg)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, boxShadow: '6px 6px 0 rgba(0,0,0,0.2)' }}>
          <span style={{ fontSize: '1.5rem', color: 'black', letterSpacing: '1px', fontFamily: 'Audiowide' }}>incl. mp3 extraction</span>
        </div>
      ...
      {/* YouTube box */}
      <div onClick={() => navigate('/media-machine/download-youtube')} style={{ position: 'absolute', left: '50%', bottom: '60px', transform: 'translateX(-50%) skewX(-4deg)', width: '500px', height: '90px', background: 'black', border: '6px solid black', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60, boxShadow: '10px 10px 0 rgba(0,0,0,0.2)', cursor: 'pointer', transition: 'transform 0.2s' }}
        onMouseEnter={(e) => e.currentTarget.style.transform = 'translateX(-50%) skewX(-4deg) scale(1.02)'}
        onMouseLeave={(e) => e.currentTarget.style.transform = 'translateX(-50%) skewX(-4deg) scale(1)'}>
        <span style={{ fontSize: '2.8rem', color: 'white', transform: 'skewX(4deg)', letterSpacing: '2px', fontFamily: 'Audiowide' }}>Or YouTube</span>
      </div>
